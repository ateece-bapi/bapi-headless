  ...
  try {
          ...
          return generateProductMetadata(
            {
              ...
              image: product.image,
              galleryImages: ('galleryImages' in product
                ? product.galleryImages?.nodes
                : null) as any,
              categories: ('productCategories' in product
                ? product.productCategories?.nodes
                : null) as any,
              averageRating: ('averageRating' in product ? product.averageRating : null) as
                | number
                | null,
    ...
    if (categoryResult.status === 'rejected' && productResult.status === 'rejected') {
      logger.error('[generateMetadata] Both queries failed', {
        slug,
        categoryError:
          categoryResult.reason instanceof Error ? categoryResult.reason.message : 'Unknown',
        productError:
          productResult.reason instanceof Error ? productResult.reason.message : 'Unknown',
      });
    }
...
export default async function ProductPage({
  ...
  try {
    ...
    if (categoryResult.status === 'fulfilled' && categoryResult.value.productCategory) {
      ...
      logger.info('[ProductPage] Category products fetched', {
        count: productsData.products?.nodes?.length || 0,
      });
    ...
    if (productResult.status === 'fulfilled') {
      try {
        ...
        if (product.__typename === 'VariableProduct') {
          try {
            ...
            const client = getGraphQLClient(
              ['product-variations', `product-${product.databaseId}`],
              true
            );
        ...
        logger.info('[ProductPage] Product transformed', {
          productDbId,
        ...
        const productSchema = generateProductSchema(
          {
            ...
            regularPrice: (product as any).regularPrice
              ? parseFloat((product as any).regularPrice)
              : undefined,
            salePrice: (product as any).salePrice
              ? parseFloat((product as any).salePrice)
              : undefined,
        ...
        return (
          <>
            ...
            <Suspense
              fallback={
                <div className="container mx-auto h-96 animate-pulse rounded bg-neutral-50 px-4 py-12" />
              }
            >
