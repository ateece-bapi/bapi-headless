...
const SignInButton: React.FC = () => {
  ...
  // Only render Clerk components on authenticated pages
  // (account, checkout, order-confirmation have ClerkProvider)
  const isAuthPage = pathname?.includes('/account') || 
                     pathname?.includes('/checkout') || 
                     pathname?.includes('/order-confirmation');
  
  // On public pages, show simple sign-in link
  if (!isAuthPage) {
    return (
      <a
        href="/sign-in"
  ...
  // On authenticated pages, use Clerk components
  return (
    <>
      <SignedOut>
